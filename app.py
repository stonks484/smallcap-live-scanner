...
def classify(text):
    ...
    bearish = ['offering','dilution','dilutive','warrant','convertible','bankruptcy','going concern']
...
async def live_scan():
    async with httpx.AsyncClient(headers={'User-Agent':'SmallCapLiveScanner/1.0'}) as client:
        ...
        rows=rows[:50]
        async def enrich(r):
            news=await get_json(client,'/v2/reference/news',{'ticker':r['ticker'],'limit':1,'sort':'published_utc','order':'desc'})
            items=(news or {}).get('results') or []
            if items:
                n=items[0]; title=n.get('title',''); kind, cs=classify(title+' '+n.get('description',''))
                r['news']=[{'title':title,'url':n.get('article_url',''),'source':(n.get('publisher') or {}).get('name',''),'published':n.get('published_utc',''),'catalyst':kind}]
                r['catalyst']=title; r['catalyst_score']=cs
            else:
                r['news']=[]; r['catalyst']='No recent news found'; r['catalyst_score']=45
...
async def loop():
    ...
    while True:
        ...
        await broadcast({'type':'scan','timestamp':last_scan_at,'rows':rows,'mode':'LIVE' if API_KEY else 'DEMO'})
